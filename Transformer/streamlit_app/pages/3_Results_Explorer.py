...
if os.path.exists(MODEL_PATH):
    try:
        ...
        st.write("Input English text to translate to Spanish.")
        user_text = st.text_input("Enter English text...", "Hello, how are you?")
        if st.button("Translate"):
            tokens = np.random.randint(0, 1000, (1, 50)) 
            pred = model.predict(tokens)
            st.success("**Translated Output:** Hola, como estas? (Dummy translation)")
